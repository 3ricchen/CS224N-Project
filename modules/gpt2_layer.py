...
class GPT2Layer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    attention_res = self.add(hidden_states, attention_output, self.attention_dense, self.attention_dropout)

    normed_ff_input = self.out_layer_norm(attention_res)
    interm_output = self.interm_af(self.interm_dense(normed_ff_input))
    res_out = self.add(attention_res, interm_output, self.out_dense, self.out_dropout)
    return res_out
